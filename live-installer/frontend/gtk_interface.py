...
class InstallerWindow:
    ...
    def i18n(self):
        ...
        self.builder.get_object("label_custom_install_paused_3").set_label(_("Install any packages that may be needed for first boot (mdadm, cryptsetup, dmraid, etc) by calling \"chroot /target\" followed by the relevant apt-get/aptitude installations."))
            ...
            if (sel == self.PAGE_WELCOME):
                ...
                if(self.setup.real_name is None or self.setup.real_name == ""):
                    ...
                    for char in self.setup.username:
                        if(char.isspace()):
                            errorFound = True
                            errorMessage = _("Your username cannot contain whitespace characters.")
                            focus_widget = self.builder.get_object("entry_username")
                            break
                    for char in self.setup.hostname:
                        if(char.isspace()):
                            errorFound = True
                            errorMessage = _("The computer's name cannot contain whitespace characters.")
                            focus_widget = self.builder.get_object("entry_hostname")
                            break
